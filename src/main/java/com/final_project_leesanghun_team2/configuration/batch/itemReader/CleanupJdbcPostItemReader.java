...
@Component
@Slf4j
public class CleanupJdbcPostItemReader implements ItemReader<List<Long>> {

	private final JdbcTemplate jdbcTemplate;

	@Autowired
	public CleanupJdbcPostItemReader(JdbcTemplate jdbcTemplate) {
	...
	public List<Long> read() throws Exception {
		...
		List<Long> ids = jdbcTemplate.queryForList(
				"SELECT post_id FROM post WHERE deleted_at IS NOT NULL AND deleted_at < ?",
